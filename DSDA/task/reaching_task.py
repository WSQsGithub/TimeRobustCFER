...
parser.add_argument('--config', type=str, help='training configuration name in options.json', default='reaching01')
parser.add_argument('--save', type=int, help='whether to save data', default=1)

args = parser.parse_args()

# example: python reaching_task.py --T 10 --tau 5 --delta 2 --gridsize 5 --alg naive --suffix 0420 --config reaching01 --save 1
...
task = Formula_task_FF(op_out='F', 
                     op_in=['F'], 
                     T=T, tau=[tau], delta=delta, target=region[0,:])

agent = Q_Agent(name=f"reaching_{args.suffix}",
              task=task,
              s0=s0)
agent.view_Qtable()
