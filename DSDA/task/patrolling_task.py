# Task FF: Phi = G_[0,T)F_[0,tau) s in A and F_[0,tau) s in B
# initialization
import sys
import os
...
parser.add_argument('--delta', type=int, help='temporal robustness threshold', default=0)
...
parser.add_argument('--config', type=str, help='training configuration name in options.json', default='patrolling01')
parser.add_argument('--save', type=int, help='whether to save data', default=1)

args = parser.parse_args()

# example: python patrolling_task.py --T 10 --tau 5 --delta 2 --gridsize 5 --alg naive --suffix 0420 --config patrolling01 --save 1
...
print(f'-----------------------T={T}, tau={tau}, delta={delta}, gridsize={grid_size}, algorithm={alg}--------------------------------')
folder_path = '../data/patrolling/'
file_name = f'{alg}_tau{tau}_delta{delta}_grid{grid_size}_{args.suffix}'
...
world = World(grid_size=grid_size, 
              region=region,  
              prob_right=0.93, 
              beta=50,scale=[1,0.5],obstacle=obstacle)

task = Formula_task_GF(op_out='G', 
                     op_in=['F','F'], 
                     T=T, tau=[tau, tau], delta=delta, target=region[0:2,:])

agent = Q_Agent(name=f"patrolling_{args.suffix}",
              task=task,
              s0=s0)
agent.view_Qtable()
