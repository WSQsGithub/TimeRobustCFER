...
class Q_Agent:
    ...
    def generate_cf_experience(self, state, f_state, action, new_state, world) -> list:
        ...
        cf_states = list(itertools.product(*[range(t) for t in self.task.tau], range(-1,self.task.delta+1))) # iterate all combinations of f states and g state
        # the result is a list of length = tau[0]*tau[1]*...*tau[n-1]*(delta+1)
        ...
        for cf_state in cf_states:
            ...
            reward = world.getReward(self.task.isTRB(new_cf_state),self.task) 
        ...
        try:
            ...
            for episode in p_bar:
                ...
                for step in range(self.task.H):
                    ...
                    # determine reward
                    reward = world.getReward(self.task.isTRB(new_f_state),self.task)
