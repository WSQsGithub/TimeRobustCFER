...
class Formula: 
    def __init__(self, op_out, op_in, T, tau, delta) -> None:
        '''
        initialize instance with number of subformulas, the outer temporal operator and the subformula operators and horizons
        '''
        self.n_flag = len(op_in)+1 # number of flag variable needed plus one that shows the temporal robust state
        self.op_out = op_out # the outer temporal operator:G/F
        self.op_in = op_in # the inner temporal opertator for each subtask: [G,F]
        self.flag = np.zeros(self.n_flag) # each flag variable, with the last flag being the temporal state
        self.flag[-1] = -1 # the temporal state is initialized as -1
        self.T = T # time bound for outer temporal operator
        self.tau = tau # horizon for each subtask
        self.delta = delta # the temporal robustness threshold
    ...
    def updateFlag(self, sat: list, flag: int) -> int:
        ...
        for i in range(0, self.n_flag-1):
            if self.op_in[i] == 'G':
                if sat[i] == 1: # determine if the ith sub-task is satisfied, if so update the subtask flag variable
        ...
        # determine if the temporal flag by counting steps of continuous satisfaction
        res = self.sat4All(new_flag)
        if res == 1:
            new_flag[-1] = min(flag[-1]+1, self.delta)
        else: 
            new_flag[-1] = -1 # -1 means that the task is not satisfied at all
        return new_flag
    ...
    def sat4Sub(self, flag) -> bool:
        ...
        '''
        sat_sub = np.zeros(self.n_flag-1)
        for i in range(self.n_flag-1):
            if self.op_in[i] == 'G':
    ...
    def sat4All(self, flag):
        raise NotImplementedError("Please Implement this method")
    
    def isTRB(self, flag):
        return (flag[-1]==self.delta)
    
           
class Formula_task_FG(Formula):
    # phi = F[0,12](G[0,3] x in A)
    def __init__(self, op_out, op_in, T, tau, delta, target):
        super().__init__(op_out, op_in, T, tau, delta)
