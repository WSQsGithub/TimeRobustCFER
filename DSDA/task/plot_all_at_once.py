...
class App():
    def __init__(self,file_path, file_name, file_dir) -> None:
        ...
        self.curve_file_dir = '../images_new/reward/'
        self.sat_file_dir = '../images_new/sat/'
...
def curveData(arr, window_size):
    ...
    lower_bounds = [max(avg - std,0) for avg, std in zip(rolling_mean, rolling_std)]
    upper_bounds = [min(avg + std,1) for avg, std in zip(rolling_mean, rolling_std)]
