...
class Formula: 
    ...
    def getReward(self,new_f_state):
        # for F formula, return reward if 
        return 1
    
    def generateCF(self, experience):
        action = experience.action
        e_state = experience.state[:-self.n_flag]
        next_e_state = experience.next_state[:-self.n_flag]
        cf_states = list(itertools.product(*[range(t) for t in self.tau]))
        ...
        for cf_state in cf_states:
            ...
            new_cf_state = self.updateFlag(next_e_state,cf_state) 
            reward = self.getReward(new_cf_state) 
